...
@property (nonatomic, assign) BOOL haveName;//可配置名字是否存在，true：存在；false：不存在，不存在时可以随意设置Signer.UName,不限制姓名的字符串类型和长度的；
...
@property (nonatomic, strong) OCRCaptureObj *ocrCaptureObj;//手写识别对象
@end
